...
func EvaluatePluginCompatibility(manifest PluginManifest, host PluginHostInfo) PluginCompatibility {
	result := PluginCompatibility{
		CurrentHeiGate:     host.Version,
		RequiredHeiGate:    manifest.Requires.HeiGate,
		RecommendedHeiGate: manifest.Requires.RecommendedHeiGateVersion,
	...
	if manifest.Requires.PluginProtocol != pluginv1.ProtocolVersion ||
		...
		result.Message = "插件协议版本与当前 HeiGate 不兼容"
		return result
	}
	if !matchesSemverRange(host.Version, manifest.Requires.HeiGate) {
		result.Status = "incompatible"
		result.Message = fmt.Sprintf("当前 HeiGate %s 不满足插件要求 %s", host.Version, manifest.Requires.HeiGate)
		return result
	}
	result.Compatible = true
	for _, tested := range manifest.Requires.TestedHeiGateVersions {
	...
	if result.Tested {
		...
		result.Message = "当前 HeiGate 版本已由插件声明测试"
	} else {
		result.Status = "untested"
		result.Message = "版本范围兼容，但插件未声明已测试当前 HeiGate 版本"
	}
	return result
}
