...
HeiGate supports long-lived SSE and WebSocket requests. Protect the request
...
CIDR/IP addresses that connect directly to HeiGate. An explicit empty list
...
`gzip_types` and do not use `gzip_types *` for HeiGate. The
...
HeiGate `server.trusted_proxies` with the Caddy address/private subnet so the
