...
Commands:
  ...
  up [--recreate]       Create and start the complete HeiGate stack
...
acquire_lock() {
    if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
        ...
        if [[ "${owner_pid}" =~ ^[0-9]+$ ]] && ! kill -0 "${owner_pid}" 2>/dev/null; then
            ...
            die "Another HeiGate Apple container operation is already running."
        fi
    fi
    printf '%s\n' "$$" >"${LOCK_DIR}/pid"
    LOCK_ACQUIRED=true
...
prepare_environment() {
    ...
    APP_IMAGE="$(read_env_value APPLE_CONTAINER_SUB2API_IMAGE ghcr.io/heiheiaoo/heigate:latest)"
...
create_app_container() {
    info "Creating HeiGate container..."
    container create \
        --name "${APP_CONTAINER}" \
        --label "${STACK_LABEL_KEY}=${STACK_LABEL_VALUE}" \
        --network "${NETWORK_NAME}" \
...
start_app() {
    ...
    if ! wait_for_probe "HeiGate" 180 probe_app; then
        show_failure_logs "${APP_CONTAINER}"
        die "HeiGate did not become ready."
    fi
    if ! wait_for_probe "HeiGate host port" 15 probe_host_app; then
...
cmd_up() {
    ...
    info "HeiGate is available at http://${ACCESS_HOST}:${HOST_PORT}"
}
...
cmd_down() {
    ...
    info "HeiGate stack stopped; persistent volumes were preserved."
}
...
confirm_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        printf 'Delete the HeiGate stack and all persistent data? [y/N] '
    else
        printf 'Delete the HeiGate containers and network, preserving volumes? [y/N] '
    fi
    read -r answer
    [[ "${answer}" == "y" || "${answer}" == "Y" ]]
}
...
cmd_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        ...
        info "HeiGate stack and persistent data deleted."
    else
        info "HeiGate stack deleted; persistent volumes were preserved."
    fi
}
