...
# 或：
#   sudo ./install-datamanagementd.sh --source /path/to/heigate/repo

BIN_PATH=""
SOURCE_PATH=""
INSTALL_DIR="/opt/heigate"
DATA_DIR="/var/lib/heigate/datamanagement"
SERVICE_FILE_NAME="heigate-datamanagementd.service"
...
参数:
  ...
  --source  指定 heigate 仓库路径（脚本会执行 go build）
  -h, --help 显示帮助

示例:
  sudo ./install-datamanagementd.sh --binary ./datamanagement/datamanagementd
  sudo ./install-datamanagementd.sh --source /opt/heigate-src
EOF
}
...
if ! id heigate >/dev/null 2>&1; then
  echo "[2/6] 创建系统用户 heigate..."
  useradd --system --no-create-home --shell /usr/sbin/nologin heigate
else
  echo "[2/6] 系统用户 heigate 已存在，跳过创建"
fi
...
echo "[4/6] 准备数据目录..."
mkdir -p "$DATA_DIR"
chown -R heigate:heigate /var/lib/heigate
chmod 0750 "$DATA_DIR"
...
systemctl enable --now heigate-datamanagementd

echo "[6/6] 完成，当前状态："
systemctl --no-pager --full status heigate-datamanagementd || true

cat <<'EOF'

下一步建议：
1. 查看日志：sudo journalctl -u heigate-datamanagementd -f
2. 在 heigate（容器部署时）挂载 socket:
