#!/bin/bash
#
# HeiGate Installation Script
# HeiGate 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/Heiheiaoo/HeiGate/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="Heiheiaoo/HeiGate"
INSTALL_DIR="/opt/heigate"
SERVICE_NAME="heigate"
SERVICE_USER="heigate"
CONFIG_DIR="/etc/heigate"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="HeiGate 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 HeiGate 服务："
    ...
    ["not_installed"]="HeiGate 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 HeiGate。"
    ...
    ["uninstall_complete"]="HeiGate 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 HeiGate"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 HeiGate"
    ...
    ["server_config_desc"]="配置 HeiGate 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="HeiGate Installation Script"
    ...
    # Completion
    ["install_complete"]="HeiGate installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start HeiGate service:"
    ...
    ["not_installed"]="HeiGate is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove HeiGate from your system."
    ...
    ["uninstall_complete"]="HeiGate has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install HeiGate"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove HeiGate"
    ...
    ["server_config_desc"]="Configure HeiGate server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/heigate" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/heigate" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="heigate_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/sub2api" "$INSTALL_DIR/heigate"
    chmod +x "$INSTALL_DIR/heigate"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/heigate"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/heigate.service << EOF
[Unit]
Description=HeiGate - AI API Gateway Platform
Documentation=https://github.com/Heiheiaoo/HeiGate
...
Type=simple
    User=heigate
    Group=heigate
    WorkingDirectory=/opt/heigate
    ExecStart=/opt/heigate/heigate
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
    SyslogIdentifier=heigate
...
start_service() {
    ...
    if systemctl start heigate; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u heigate -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable heigate 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status heigate"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u heigate -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart heigate"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop heigate"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/heigate" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet heigate; then
        print_info "$(msg 'stopping_service')"
        systemctl stop heigate
    fi

    # Backup current binary
    cp "$INSTALL_DIR/heigate" "$INSTALL_DIR/heigate.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/heigate.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/heigate"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start heigate

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: HeiGate must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/heigate" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="heigate.backup.${current_version}"
        else
            backup_name="heigate.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/heigate" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop heigate 2>/dev/null || true
    systemctl disable heigate 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/heigate.service
    systemctl daemon-reload
