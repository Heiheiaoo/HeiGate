...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready HeiGate setups.
  ...
  const configContent = `# Grok Build CLI → HeiGate Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for HeiGate Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches HeiGate).
...
description = "Grok 4.5 via HeiGate (Responses)"
...
# Imagine tools: model IDs go to HeiGate media endpoints (not the text [model.*] catalog).
...
# Grok/HeiGate path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at HeiGate OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via HeiGate'
