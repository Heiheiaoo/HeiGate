...
export function resolveDocumentTitle(routeTitle: unknown, siteName?: string, titleKey?: string): string {
  const normalizedSiteName = typeof siteName === 'string' && siteName.trim() ? siteName.trim() : 'HeiGate'
...
export function resolveRouteDocumentTitle(
  ...
  if (route.name === 'DesktopPreview' || route.meta?.title === 'HeiGate' || route.meta?.title === 'HeiGate Desktop') {
    return 'HeiGate'
  }
